...
def test_writer_doesnt_change_feature_locations(tmpdir):
    parser = CrazydocParser(["highlight_color", "bold", "underline", "font_color", "lower_case"])
    ...
    output_path = os.path.join(str(tmpdir), "test_writer_doesnt_change_feature_locations.docx")
    ...
    for seq, written_seq in zip(seqs, written_seqs):
        ...
        locations.sort()
        written_locations = [(int(f.location.start),int(f.location.end)) for f in written_seq.features]
